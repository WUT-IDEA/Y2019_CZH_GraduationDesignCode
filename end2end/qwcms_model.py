...
embedding_maxtrix = np.loadtxt("../../data/mine_glove.txt", dtype=np.float32)
...
wordlstm = LSTM(units=config.encdim, return_sequences=True, return_state=True)
qhs, qh, qc = wordlstm(q_embd_layer)
shs, sh, sc = wordlstm(s_embd_layer)
rhs, rh, rc = wordlstm(r_embd_layer)
...
model.load_weights("model_multi4/model_40.h5")
...
logpath = "logs_multi5.txt"
modelpath = "model_multi5"
